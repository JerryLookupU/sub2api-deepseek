...
// 再由 GetDefaultPlatformQuotas（读）正确往返——覆盖真实 write→read 路径，锁住全平台补齐契约。
func TestSystemPlatformQuotas_WriteReadRoundTrip(t *testing.T) {
	...
	// 全平台补齐契约：无论写了几个 platform，读回必须含全部允许平台
	for _, p := range AllowedQuotaPlatforms {
		if _, ok := got[p]; !ok {
			t.Errorf("all-platform contract violated: missing platform %q", p)
		}
	}
	// 写入值正确往返
...
func TestSystemPlatformQuotas_EmptyMapClearsAll(t *testing.T) {
	...
	for _, p := range AllowedQuotaPlatforms {
		if _, ok := got[p]; !ok {
			t.Errorf("all-platform contract violated after empty write: missing %q", p)
		}
	}
	// 所有字段 nil（全部已清空）
	for _, p := range AllowedQuotaPlatforms {
