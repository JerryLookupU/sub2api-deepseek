...
describe('AccountUsageCell', () => {
  ...
  it('OpenAI OAuth 快照已过期时首屏会重新请求 usage', async () => {
    ...
    expect(getUsage).toHaveBeenCalledWith(2000, undefined, undefined)
  ...
  it('OpenAI OAuth 有 codex 快照时仍然使用 /usage API 数据渲染', async () => {
    ...
    expect(getUsage).toHaveBeenCalledWith(2001, undefined, undefined)
  ...
  it('OpenAI OAuth 有现成快照时，手动刷新信号会触发 usage 重拉', async () => {
    ...
    expect(getUsage).toHaveBeenLastCalledWith(2010, undefined, true)
  ...
  it('OpenAI OAuth 在无 codex 快照时会回退显示 usage 接口窗口', async () => {
	...
	expect(getUsage).toHaveBeenCalledWith(2002, undefined, undefined)
  ...
  expect(getUsage).toHaveBeenCalledWith(2004, undefined, undefined)
